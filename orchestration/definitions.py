...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "extract_load"))
from load_csv import r2_source
...
# create dlt asset 
@dlt_assets(
    dlt_source = r2_source(),
    dlt_pipeline = dlt.pipeline(
        pipeline_name="dependency_r2",
        dataset_name="sch_dlt_test",
...
dbt_project_directory = Path(__file__).parents[1] / "data_transformation" / "dbt_demo_core"
# Define the path to your profiles.yml file (in your home directory)
profiles_dir = "C:/Users/oojameru/OneDrive - Best Buy/Documents/vscode/dbt_sandbox/"
...
job_dlt = dg.define_asset_job("job_dlt", selection=dg.AssetSelection.keys("dlt_r2_source_load_csv_resource"))
...
@dg.asset_sensor(asset_key=dg.AssetKey("dlt_r2_source_load_csv_resource"),
                 job_name="job_dbt")
def dlt_load_sensor():
    yield dg.RunRequest()
