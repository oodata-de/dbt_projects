...
FROM {{ ref('prv_inv__inventory_levels') }} il
LEFT JOIN {{ ref('prv_sales__products') }} p ON il.product_id = p.product_id
LEFT JOIN {{ ref('prv_inv__inventory_transactions') }} it ON il.product_id = it.product_id 
